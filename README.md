...
I finally found to exploit some ML as well, with the logistic regression to identify the slope and to filter out the train routes that were getting far; and with the selection of candidates that is basically an NN approach (Nearest Neighbors), quite basic but often very useful.
Another reason that supported my final decision is what I found in reading, in the dedicated forum of the challenge, ideas and approaches shared by few of the participants. Among those I did not find many ML approaches, probably due to the type of challenge. However, to be honest, I do strongly believe that with the given features some approaches will significantly improve, if not as a standalone methods maybe as a candidate selection. I'll discuss a little more about this latter point in the Next Development.
